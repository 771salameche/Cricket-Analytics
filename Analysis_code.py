import pandas as pd 
df_delivries = pd.read_csv("innings_deliveries.csv")
print(df_delivries.head())

import matplotlib.pyplot as plt
import seaborn as sns
sns.set_style('whitegrid')
#data prepartion for run distribution per over 
run_distribution = df_delivries.groupby(['team', 'over']).agg({'runs_total': 'sum'}).reset_index()
...
top_scorers = df_delivries.groupby(['team', 'batter']).agg({'runs_batter': 'sum'}).reset_index().sort_values(by='runs_batter', ascending=False)
...
df_delivries['wickets_taken'] = df_delivries['wicket_kind'].notna().astype(int)
bowling_stats = df_delivries.groupby(['team', 'bowler']).agg({'runs_total': 'sum', 'wickets_taken': 'sum', 'over': 'nunique'}).reset_index()
...
bowling_stats_sorted['wickets_taken'] = df_delivries['wicket_kind'].notna().astype(int)
bowling_stats = df_delivries.groupby(['team', 'bowler']).agg({'runs_total': 'sum', 'wickets_taken': 'sum', 'over': 'nunique'}).reset_index()
...
dismissal_types = df_delivries['wicket_kind'].dropna().value_counts()
...
partnerships_data = calculate_partnerships(df_delivries)
...
df_delivries['phase'] = df_delivries['over'].apply(classify_phase)

# grouping data by phase and team to calculate runs and wickets
phase_analysis = df_delivries.groupby(['team', 'phase']).agg({'runs_total': 'sum', 'wickets_taken': 'sum', 'over': 'count'}).rename(columns={'over': 'balls'}).reset_index()
...
batter_stats = df_delivries.groupby('batter').agg({'runs_batter': 'sum', 'over': 'count'}).rename(columns={'over': 'balls_faced'}).reset_index()
...
batter_phase_stats = df_delivries.groupby(['batter', 'phase']).agg({'runs_batter': 'sum', 'over': 'count'}).rename(columns={'over': 'balls_faced'}).reset_index()
...
df_delivries['cumulative_runs'] = df_delivries.groupby('team')['runs_total'].cumsum()
df_delivries['cumulative_wickets'] = df_delivries.groupby('team')['wickets_taken'].cumsum()

# separate data for both teams
rcb_deliveries = df_delivries[df_delivries['team'] == 'Royal Challengers Bengaluru']
dc_deliveries = df_delivries[df_delivries['team'] == 'Delhi Capitals']
...
per_over_stats = df_delivries.groupby(['team', 'over']).agg({'runs_total': 'sum', 'wickets_taken': 'sum'}).reset_index()
